...
{
    printf("uuid=%x",p_ble_evt->evt.gatts_evt.params.write.uuid.uuid);
    ...
    if ((p_evt_write->handle == p_cus->custom_value_handles_A.cccd_handle)
        && (p_evt_write->len == 2)
       )
    {
...
static uint32_t custom_value_char_add(ble_cus_t * p_cus, const ble_cus_init_t * p_cus_init)
{
    ...
    ble_uuid.uuid = CUSTOM_VALUE_CHAR_UUID_A;
    ...
    ble_uuid.uuid = CUSTOM_VALUE_CHAR_UUID_B;
...
uint32_t ble_cus_custom_value_update(ble_cus_t * p_cus, uint8_t *custom_value)
{
      ...
      err_code = sd_ble_gatts_value_set(p_cus->conn_handle,
                                        p_cus->custom_value_handles_A.value_handle,
                                        &gatts_value);
      if (err_code != NRF_SUCCESS)
      {
          return err_code;
     ...
     if(dataready){
      ...
      if ((p_cus->conn_handle != BLE_CONN_HANDLE_INVALID)) 
      {
          ...
          hvx_params.handle = p_cus->custom_value_handles_A.value_handle;
      ...
      {
      NRF_LOG_INFO("waiting for data(0x01) from char B");
      }
